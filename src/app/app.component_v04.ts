...
private searchScope = new Set<string>();
 ...
 private searchInCache(term: string): any[] {
  ...
  for (const node of this.nodeMap.values()) {
    ...
    if (
      ...
      node.firstName?.toLowerCase().includes(lower)
    ) {
      results.push(node);
    }
  }
...
async searchNode(term: string) {
  ...
  /* 0️⃣ CACHE-FIRST SEARCH */
  let matches = this.searchInCache(term);

  /* 0️⃣b API FALLBACK ONLY IF NEEDED */
  if (!matches.length) {
    matches = await firstValueFrom(this.repository.search(term));
    if (!matches.length) {
      alert('Not found');
      return;
    }
  ...
  /* 1️⃣ REGISTER MATCHES + ANCESTORS */
  for (const match of matches) {
    this.registerNode(match);

    let cur: any = match;
    while (cur?.parentId) {
      cur = await this.ensureNode(cur.parentId);
      if (!cur) break;
    }
  }

  /* 2️⃣ BUILD SEARCH SCOPE (CACHE-FIRST) */
  ...
  for (const match of matches) {
    ...
    while (cur) {
      ...
      if (cur.parentId) {
        const siblings = await this.ensureChildren(cur.parentId);
...
private async ensureNode(id: string): Promise<any | null> {
  let node = this.nodeMap.get(id);
  if (node) return node;

  const res = await firstValueFrom(
    this.repository.get(`/api/nodes/${id}`)
  );
...
private async ensureChildren(parentId: string): Promise<any[]> {
  let children = this.cache.get(parentId);
  if (children) return children;

  children = await firstValueFrom(
    this.repository.get(`/api/nodes/${parentId}`)
  );
